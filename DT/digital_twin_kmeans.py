...
# ── Configuración de optimización automática de topología ─────────────────────
# Optimización periódica con K-Means ponderado por baja cobertura (SINR bajo)
ENABLE_TOPOLOGY_OPTIMIZATION = True
OPTIMIZATION_INTERVAL = 10.0          # Segundos de simulación entre optimizaciones
OPTIMIZATION_MIN_DISTANCE = 50.0      # Distancia mínima para enviar MOVE
OPTIMIZATION_SPEED = 20.0             # Velocidad de vuelo del dron en m/s
OPTIMIZATION_MIN_UES = 1              # Mínimo de UEs para considerar optimización
...
KMEANS_MAX_MOVE_PER_CYCLE = 300.0     # Limita distancia por ciclo para estabilidad
# ────────────────────────────────────────────────────────────────────────────────
...
class DigitalTwin:
    ...
    def _sinr_priority_weight(self, sinr):
        """
        Peso de prioridad:
        peor SINR => mayor peso => K-Means prioriza esa zona.
        """
        if sinr < -10:
            return 5.0
        elif sinr < 0:
    ...
    def _collect_weighted_ue_points(self):
        """
        Construye dataset de clustering:
        X: puntos (x, y) de UEs
        W: pesos por SINR
        """
        pts = []
        ...
        for _, nd in self.G.nodes(data=True):
            ...
            x = nd.get('pos_x', None)
            y = nd.get('pos_y', None)
            sinr = nd.get('sinr', -999)

            if x is None or y is None:
                continue

            pts.append([float(x), float(y)])
            wts.append(float(self._sinr_priority_weight(float(sinr))))
    ...
    def _get_mobile_gnbs(self):
        """
        En tu escenario actual: todas las estaciones son móviles.
        """
        drones = []
    ...
    def _limit_step_towards_target(self, x0, y0, xt, yt, max_step):
        """
        Limita el desplazamiento máximo por ciclo.
        """
        dx = xt - x0
        dy = yt - y0
    ...
    def _assign_drones_to_centers_greedy(self, drones, centers):
        """
        Asigna cada centroide al dron más cercano (greedy).
        """
        ...
        for c in centers:
            ...
            best_i = None
            best_d = float('inf')
            for i, d in enumerate(remaining_drones):
                dist = self._dist(d['x'], d['y'], cx, cy)
                if dist < best_d:
                    best_d = dist
                    best_i = i

            chosen = remaining_drones.pop(best_i)
    ...
    def compute_optimal_positions(self):
        """
        Optimización periódica basada en K-Means ponderado por SINR.
        - Todas las estaciones son móviles.
        - Prioriza zonas de baja cobertura.
        - Incluye histéresis y límite de movimiento por ciclo.
        """
        moves = []

        drones = self._get_mobile_gnbs()
        if not drones:
            print("[KMEANS] No hay gNodeBs móviles disponibles.")
            return moves
        ...
        # Warm start con posiciones actuales de drones
        ...
        for drone, cx, cy, raw_dist in assignments:
            ...
            print(
                f"[KMEANS-OPTIM] gNodeB[{drone['gnb_index']}] "
