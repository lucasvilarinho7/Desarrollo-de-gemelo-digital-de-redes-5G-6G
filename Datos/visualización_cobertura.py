...
# Cargar shapefile y filtrar Madrid

gdf_esp = gpd.read_file("gadm41_ESP_shp/gadm41_ESP_1.shp") # Descargar de la web oficial de GADM
gdf_madrid_poly = gdf_esp[gdf_esp["NAME_1"] == "Comunidad de Madrid"] # Filtrar solo el área de la Comunidad de Madrid

# Cargar CSV con estaciones en este caso LTE, al menos de momento

df_madrid = pd.read_csv("LTE_stations_Spain.csv")  
...
madrid_polygon = gdf_madrid_poly.geometry.union_all()
gdf_stations_inside = gdf_stations[gdf_stations.within(madrid_polygon)]
gdf_stations_inside_filtrada = gdf_stations_inside.drop(columns=['geometry'], axis=1)
gdf_stations_inside_filtrada.to_csv("estaciones_lte_madrid.csv", index=False)

# Reproyectar a proyección métrica (UTM 30N)

gdf_stations_inside = gdf_stations_inside.to_crs(epsg=25830)
gdf_madrid_poly = gdf_madrid_poly.to_crs(epsg=25830)
...
# Polígono de Madrid
gdf_madrid_poly.plot(ax=ax, color='white', edgecolor='black')
...
ax.set_title("Cobertura Estaciones LTE - Comunidad de Madrid", fontsize=14)
ax.set_axis_off()
plt.show()
