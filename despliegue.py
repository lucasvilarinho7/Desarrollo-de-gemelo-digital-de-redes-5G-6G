...
def graph_generator():
    ...
    for i, row in enumerate(df_stations.itertuples(), start=1):
        G.add_node(f"A_{i}", pos=(row.lat, row.lon), type="antenna", range=row.range)
...
def visualize_graph(G):
    ...
    nx.draw(G, pos, with_labels=True, font_size=8, node_color=node_colors, edge_color="gray", node_size=100)
